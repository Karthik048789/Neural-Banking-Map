...
export const LiveFeed = () => {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        const newTxn = {
          ...rawTxn,
          txn_id: rawTxn.txn_id || `TXN_${Math.random().toString(36).substring(2, 10).toUpperCase()}`,
          type: rawTxn.type || 'UPI',
          confidence: rawTxn.confidence <= 1
            ? Math.round(rawTxn.confidence * 100)
            : rawTxn.confidence,
        };
  ...
  return (
    <div className="p-4 md:p-8 max-w-[1600px] mx-auto flex h-full md:h-[calc(100vh-2rem)] flex-col gap-6">
      <Toaster position={typeof window !== 'undefined' && window.innerWidth < 768 ? "bottom-center" : "top-right"} />
